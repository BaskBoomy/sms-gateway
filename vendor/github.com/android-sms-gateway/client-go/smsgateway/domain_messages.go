...
// PATCHED: BaskBoomy MMS Attachment passthrough (v1.14.0-mms.1). Re-apply after any `go mod vendor`.
...
type TextMessage struct {
	...
	// Attachments are optional MMS media parts. When present, the device sends the message as MMS.
	// Korean MMS limit: max 3 parts (per-part decoded size checked server-side).
	Attachments []Attachment `json:"attachments,omitempty" validate:"omitempty,max=3,dive"`
}

// Attachment is a single MMS media part carried opaquely from consumer to device.
//
// The server only stores and relays it; MMS composition happens on the device.
type Attachment struct {
	// ContentType is the MIME type. Korean carriers route LMS/MMS by this; only images are accepted.
	ContentType string `json:"contentType" validate:"required,oneof=image/jpeg image/png image/gif" example:"image/jpeg"`
	// Data is the base64-encoded media bytes.
	Data string `json:"data" validate:"required,base64" format:"byte"`
	// FileName is an optional original file name.
	FileName string `json:"filename,omitempty" validate:"omitempty,max=255" example:"photo.jpg"`
}
